...
@property (nonatomic,strong) TWDownloader * downloader;
@end

@implementation TWImageManager

static TWImageManager * imageManger = nil;
+(instancetype)shareInstance{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!imageManger) {
            imageManger = [[TWImageManager alloc] init];
        }
    });
    return imageManger;
}
-(instancetype)init{
    if (self = [super init]) {
        ...
        _downloader = [TWDownloader shareInstance];
...
-(TWDownloadOperation *)loadImageWithUrl:(NSString *)urlStr Completed:(TWloadImageCompletedBlock)completedBlock{
    ...
    __block TWDownloadOperation * operation = [[TWDownloadOperation alloc] init];
    BOOL isFailedUrl = NO;
    ...
    if (!imageData) {
        ...
        @synchronized(self.runOperationsDic){
            if (self.runOperationsDic[key]) {
                ...
                operation = [TWDownloader.shareInstance downloadImageWithUrl:url CompletedBlock:^(NSData *data, NSError *error, BOOL finished) {
                    if (!operation.isCancelled) {
                        if (!error && data) {
                            dispatch_async(dispatch_get_main_queue(), ^{
                                UIImage * img = [UIImage imageWithData:data];
